...
uint16_t map(uint16_t x, uint16_t in_min, uint16_t in_max, uint16_t out_min, uint16_t out_max) {
    return (((out_max - out_min) * (uint32_t)(x - in_min)) / (in_max - in_min)) + out_min;
}

void main(void){
   uint16_t ADC_1=0;    //Variable local
   uint16_t T =0;       //Variable local
   ...
   lcd_putc("\f");   //Limpiar pantalla LCD
   lcd_gotoxy(1,1);  //Posicionar mensaje
   lcd_putc("Temperatura");   //Escribir mensaje
   while(true){
      set_adc_channel(0);     // Selecciona el canal AN0 del ADC
      ADC_1 = read_adc();     //Leer dato ADC
      delay_us(20);           //Tiempo de muestreo
      T=map(ADC_1,0,1023,0,500); //map(dato_que_varia,rango_menor_adc,rango_mayor_adc,rango_menor_a_convertir,rango_mayor_a_convertir)
      lcd_gotoxy(1,2);           //Posiciona el mensaje
      printf(lcd_putc,"ADC:%4Lu T:%4Ld",ADC_1,T);  //imprime el mensaje
   }
}
